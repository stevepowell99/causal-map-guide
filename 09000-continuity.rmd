...
"*Tracing threads*" generalises the continuity idea to whole paths, which can be much more useful. If your map makes it look like Cash Transfer causally influenced Increased Wellbeing further downstream of it, you surely want to know if there are at least some sources who each mentioned the whole of one of the possible paths.  

We call this "tracing threads" as opposed to "tracing paths". So a thread is a chain of individual "original links" inside the bundles, whereas a path is a chain of bundles.
...
We can trace downstream or upstream. So we can say "how many threads make it all the way down to Improved Wellbeing". The threads are named by the source ids, so we can say "Let's look at downstream thread MXFG-1".

The disadvantage of tracing threads as opposed to just showing continuity is that you have to anchor the filter to one or more factors, and if you use thread tracing to format an aspect of the map (e.g. to colour the links) then this formatting is only related to this one specific set of origin and/or target factors.
...
This should be easy to understand: it's like ink starting from C and flowing downstream as long as there is source continuity.

There is a direct counterpart, *tracing upstream threads* which asks about causes of effects rather than effects of causes: "Increased wellbeing" is completely red, and the redness of "Cash Transfer C" depends on the number of sources which mention a complete path from C to Increased wellbeing. 
